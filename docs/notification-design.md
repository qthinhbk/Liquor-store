...
- **Credential resolution**: an injectable `CredentialResolver` resolves `credentialRef`. Task 4 ships only the `env://` resolver (`env://NAME` reads process environment variable `NAME`, trimmed). `render-secret://` is intentionally **fail-closed** with a stable permanent error until a deployment-specific Render resolver exists; schema validation still accepts the scheme. Raw tokens or unknown schemes are rejected before any network call.
